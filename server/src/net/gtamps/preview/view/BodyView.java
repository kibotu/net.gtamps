...
public class BodyView extends PhysicsView {
	...
	public BodyView(final Body body) {
		this.body = body;
		final Vec2 rotatedVelocity = new Vec2(body.getLinearVelocity());
		final float rotation = body.getAngle();
		rotatedVelocity.set((float)(Math.cos(rotation)*rotatedVelocity.x+Math.sin(rotation)*rotatedVelocity.y),
				(float)(-Math.sin(rotation)*rotatedVelocity.x+Math.cos(rotation)*rotatedVelocity.y));
		addChild(new VectorView(rotatedVelocity.mul(10f), body.getLocalCenter()));
