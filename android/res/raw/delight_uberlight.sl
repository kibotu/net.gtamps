/****************************************************************************
* *********** delight_uberlight.sl - a light with many fun controls. This is essentially the 
 * shader provided in the book The Renderman Shading Language Guide - Rudy Cortes, Saty Raghavachary. Just 
 * some tweaks so it works in 3delight. 
 *
 * Description:
...
delight_uberlight(
			   ...
			   string slideName[] = {};
			   uniform float slideScaleX[] = {},slideScaleY[] = {};
			   uniform float slideOffsetX[]={}, slideOffsetY[]= {};
			   uniform float slideBlur[] = {};
			   /* Noisy light */
			   ...
			   uniform float doTransmission = 0, filtWshadCol = 0,transSamples = 16;
			   uniform float transMinSamples = 4, transBlur = 0.01;
			   uniform float transBias = 0.01;
			   /* Fake blocker shadow */
			   string blockerCoords[4] = {"","","",""};
			   uniform float blockerOpac[4]={1,1,1,1};
			   float blockerwidth[4]={1,1,1,1}, blockerheight[4]={1,1,1,1};
			   float blockerwedge[4]={.1,.1,.1,.1}, 
			   blockerhedge[4]={.1,.1,.1,.1}, blockerround[4]={1,1,1,1};
			   ...
			   output float __foglight = 1; )
{
	...
	for (i = 0; i < arraylength(slideName); i += 1){
	  if (slideName[i] != "") {
	    point Pslide = PL / point (width+wedge, height+hedge, 1);
	    float zslide = zcomp(Pslide);
	    float xslide = slideOffsetX[i]+(0.5+0.5*xcomp(Pslide)/zslide);
	    float yslide = slideOffsetY[i]+(0.5-0.5*ycomp(Pslide)/zslide);
	    string tmp = slideName[i];
        lcol *= color texture (tmp, xslide * 1/slideScaleX[i],
		yslide * 1/slideScaleY[i],"blur",slideBlur[i]);
	  }
	}
...
#pragma nolint
		...
		/* Apply transmission shadows */
		if (doTransmission == 1){
		  lc = transmission(Ps, from,
...
unoccluded *= (lc[0]+lc[1]+lc[2]) / 3;
		...
		for (i = 0; i < arraylength(blockerCoords); i +=1){
		  if (blockerCoords[i] != "") {
			float block = BlockerContribution (Ps, shadoworigin,
											   blockerCoords[i],blockerwidth[i],
											   blockerheight[i],blockerwedge[i],
											   blockerhedge[i],blockerround[i]);
			unoccluded *= clamp(1,0,block + (1 - blockerOpac[i]));
		  }
		}
		
		if (filtWshadCol == 0 && doTransmission == 1)
		  lcol *= lc;
		else if (filtWshadCol == 1 && doTransmission == 1)
		  lcol = mix (shadowcolor * lc, lcol  , unoccluded);
		else
