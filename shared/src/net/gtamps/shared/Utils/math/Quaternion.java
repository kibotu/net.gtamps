...
// method
    ...
    public void toMatrix(Matrix4 matrix) {
        ...
        matrix.set(1 - 2 * (yy + zz), 2 * (xy - zw), 2 * (xz + yw), 0,
...
// final float m11, final float m12, final float m20, final float m21, final float m22
    ...
    public Quaternion setFromMatrix(Matrix4 matrix) {
        return setFromAxes(matrix.values[0], matrix.values[1], matrix.values[2],
                matrix.values[4], matrix.values[5], matrix.values[6],
                matrix.values[9], matrix.values[10], matrix.values[11]);
    }
     ...
     * @return this quaternion for chaining
     */
    public Quaternion slerpInPlace(Quaternion end, float alpha) {
        if (this.equals(end)) {
            return this;
        }
    ...
    public Vector3 asEulerAngles() {
        Vector3 e = Vector3.createNew();

//        2*acos/q0 *180 / pi, q1,q2,q3
        double test = x * y + z * w;
        if (test > 0.499) { // singularity at north pole
            e.x = (float) (2 * Math.atan2(x, w));
            e.y = (float) (Math.PI / 2);
            e.z = 0;
            return e;
        }
        ...
        double sqy = y * y;
        double sqz = z * z;
        e.x = (float) Math.atan2(2 * y * w - 2 * x * z, 1 - 2 * sqy - 2 * sqz);
        e.y = (float) Math.asin(2 * test);
        e.z = (float) Math.atan2(2 * x * w - 2 * y * z, 1 - 2 * sqx - 2 * sqz);
    ...
    public float[] toAxisAngle() {
        ...
        result[3] = 2.0f * (float) Math.acos(w); // angle
